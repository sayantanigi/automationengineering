...
class Home extends MY_Controller {
	...
	public function home_list() {
		try {
			...
			$get_users = $this->db->query("SELECT users.*, employer_subscription.employer_id FROM users JOIN employer_subscription ON employer_subscription.employer_id = users.userId WHERE users.userType = '1' AND users.status = '1' AND users.email_verified = '1' AND users.firstname != '' AND users.lastname != '' AND users.email != '' AND users.gender != '' AND users.address != '' AND users.short_bio != '' GROUP BY users.userId ORDER BY users.userId DESC")->result_array();
			if (!empty($get_users)) {
				foreach ($get_users as $key => $val) {
					if (!empty($val['profilePic'])) {
						$val['profilePic'] = base_url() . 'uploads/users/' . $val['profilePic'];
					} else {
						$val['profilePic'] = base_url() . 'uploads/no_image.png';
					}
					$return1[$key] = $val;
				}
			}
			...
			$get_ourservice = $this->db->query("SELECT category.id, category.category_name, our_service.image, our_service.icon, our_service.description, category.status, category.created_date, category.update_date FROM our_service JOIN category ON category.id = our_service.category_id")->result_array();
			if (!empty($get_ourservice)) {
				foreach ($get_ourservice as $key => $val) {
					$val1['id'] = $val['id'];
					$val1['category_id'] = $val['category_name'];
					$val1['image'] = $val['image'];
					$val1['icon'] = base_url() . 'uploads/services/' . $val['icon'];
					$val1['description'] = $val['description'];
					$val1['status'] = $val['status'];
					$val1['created_date'] = $val['created_date'];
					$val1['update_date'] = $val['update_date'];
					$return2[$key] = $val1;
				}
			}
			$data['get_ourservice'] = $return2;
		...
		try {
			...
			foreach($get_banner as $key => $arr) {
				if(!empty($arr['image'])) {
					...
					$arr['image'] = base_url().'uploads/users/user.png';
				}
				$return[$key] = $arr;
			}
	...
	public function search_job() {
		...
		try {
			...
			@$search_location = $formdata['specialist'];
