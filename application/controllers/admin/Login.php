...
class Login extends MY_Controller {
	...
	public function logOut() {
		unset($_SESSION['automation_admin']);
		redirect(admin_url());
	}

	function profile() {
		$get_admin=$this->Crud_model->get_single('admin',"userId ='".$_SESSION['automation_admin']['id']."'");
	...
	function change_password() {
		$get_user=$this->Crud_model->get_single('admin',"userId='".$_SESSION['automation_admin']['id']."'");
	    if($get_user->password==md5($_POST['cur_password'])) {
			$data=array(
				'password'=>md5($_POST['new_password'])
			);
			$this->Crud_model->SaveData('admin',$data,"userId='".$_SESSION['automation_admin']['id']."'");
