...
class Payment extends MY_Controller {
    ...
    function ajax_manage_page() {
        ...
        foreach ($GetData as $row) {
            ...
            $btn = '<button data-transaction_id="'.$row->transaction_id.'" class="btn btn-sm bg-success-light mr-2" type="button" onClick="view_detail(\''.$row->transaction_id.'\',\''.$status.'\',\''.$row->invoice_pdf.'\');"><i class="far fa-eye mr-1"></i></button>';
            $btn .= '<button class="btn btn-sm btn-danger mr-2" type="button" onClick="delete_detail(\''.$row->id.'\');"><i class="fa fa-trash mr-1"></i></button>';
            $no++;
            $nestedData = array();
            $nestedData[] = $no;
    ...
    public function delete_payment() {
        if(isset($_POST['id'])) {
            $this->Crud_model->DeleteData('employer_subscription',"id='".$_POST['id']."'");
