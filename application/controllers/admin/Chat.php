...
class Chat extends MY_Controller {
	...
	function index() {
		...
		$data['chat_list'] = $this->db->query('SELECT DISTINCT `chat`.`postjob_id`, `chat`.`id`, `chat`.`userto_id`, `chat`.`userfrom_id`, `chat`.`message`, `users`.`username`, CONCAT(users.firstname, " ", users.lastname) as full_name, `users`.`profilePic`, `to_user`.`username` as `to_username`, CONCAT(to_user.firstname, " ", to_user.lastname) as to_fullname FROM `chat` JOIN `users` ON `users`.`userId`=`chat`.`userfrom_id` JOIN `users` `to_user` ON `to_user`.`userId`=`chat`.`userto_id` order by id DESC')->result_array();
	...
	function count_all($cond) {
        ...
        $query = $this->db->query('SELECT DISTINCT(`chat`.`postjob_id`), `chat`.`id`, `chat`.`userto_id`, `chat`.`userfrom_id`, `chat`.`message`, `users`.`username`, CONCAT(users.firstname, " ", users.lastname) as full_name, `users`.`profilePic`, `to_user`.`username` as `to_username`, CONCAT(to_user.firstname, " ", to_user.lastname) as to_fullname FROM `chat` JOIN `users` ON `users`.`userId`=`chat`.`userfrom_id` JOIN `users` `to_user` ON `to_user`.`userId`=`chat`.`userto_id` order by id DESC');
	...
	function ajax_manage_page() {
		...
		$GetData = $this->db->query('SELECT DISTINCT(`chat`.`postjob_id`), `chat`.`id`, `chat`.`userto_id`, `chat`.`userfrom_id`, `chat`.`message`, `chat`.`created_date` FROM `chat` JOIN `users` ON `users`.`userId`=`chat`.`userfrom_id` JOIN `users` `to_user` ON `to_user`.`userId`=`chat`.`userto_id` order by id DESC')->result_array();
		$no=0;
		$data = array();
		foreach ($GetData as $row) {
			$btn = '<span class="btn btn-sm bg-success-light mr-2" data-toggle="modal" data-target="#viewModal" onclick="view_data(3)" data-placement="right"><i class="far fa-eye mr-1"></i><a href="'.admin_url('chat_details/'.$row['userfrom_id'].'/'.$row['userto_id']).'"></a></span>';
			$btn .= '<button class="btn btn-sm btn-danger mr-2" type="button" onClick="delete_detail(\''.$row['postjob_id'].'\');"><i class="fa fa-trash mr-1"></i></button>';
	...
	public function delete() {
        if(isset($_POST['id'])) {
            $this->Crud_model->DeleteData('chat',"id='".$_POST['id']."'");
